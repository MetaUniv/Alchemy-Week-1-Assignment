...
function getAddress(publicKey) {
    let hash = keccak256(publicKey.slice(1)).slice(-20);
...
function hashMessage(message) {
 ...
 function getSignature(privateKey) {
  const messageHash = hashMessage('Approving transaction');
  let response =  secp.sign(messageHash, privateKey, {recovered: true});
  return response
}
